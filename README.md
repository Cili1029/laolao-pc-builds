...
- **多类型通知**：支持评论通知、回复通知、点赞通知和订单状态通知（发货/到货）。
- **实时推送**：结合 WebSocket 实现消息的实时推送，确保用户及时收到通知。
- **未读计数**：提供未读消息计数功能，用户可快速查看待处理通知。
- **通知管理**：用户可查看通知列表，并支持将通知标记为已读。
- **异步处理**：通知的生成和推送采用异步方式处理，不影响主业务流程。
